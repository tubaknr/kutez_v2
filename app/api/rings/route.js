...
export async function GET(req) {
  try {
    ...
    const minPopularity =
      parseFloat(url.searchParams.get("minPopularity")) || 0;
    const maxWeight =
      parseFloat(url.searchParams.get("maxWeight")) || Nuumber.MAX_VALUE;
    ...
    return NextResponse.json(filteredProds, { status: 200 });
  } catch (error) {
    console.error("Error fetching the products: ", error);
    return NextResponse.json(
      { error: "internal server error in api/rings/route.js" },
      { status: 500 }
    );
  }
}
